...
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, BaseTlb* _itlb ,
				BaseTlb* _dtlb , g_string& _name, uint32_t _id) :Core(_name),				 l1i(_l1i), l1d(_l1d) , itlb(_itlb) , dtlb(_dtlb) , 
				id(_id), cRec(0, _name)
{
...
inline void OOOCore::load(Address addr) {
	//modified by YuJieChen on 2015.10.26
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
	...
	//std::cout<<"simulate current bbl ifetch"<<std::endl;
...
ADDRINT OOOCore::TlbTranslate( ADDRINT virtual_addr , bool is_inst, bool is_write)
{
	...
	tlb_access_num++;
	BaseTlb* tlb = NULL;
	//std::cout<<name<<" va:"<<std::hex<<virtual_addr<<std::endl;
	//instruction tlb
	if( is_inst )
	{
		//std::cout<<name<<" ins tlb. itlb ptr:"<<std::hex<<this->itlb<<" virtual address:"<<std::hex<<virtual_addr<<std::endl;
		tlb = itlb;
	}
	else
	{
		//std::cout<<name<<" data tlb. dtlb ptr:"<<std::hex<<this->dtlb<<" virtual address:"<<std::hex<<virtual_addr<<std::endl;
		tlb = dtlb;
	}
	if( tlb == NULL)
	{
	...
	//std::cout<<name<<" access tlb:"<<tlb->getName()<<std::endl;
	//std::cout<<name<<"current process id:"<<procIdx<<std::endl;
	Address addr = tlb->access(req);
	//Address addr = virtual_addr;
	curCycle = req.cycle;	//get updated cycle
	//Address line_addr= ((addr>>12)<<12);
...
//ADDRINT OOOCore::RandomizeAddress(MemReq &req) {
	//ADDRINT vAddr = req.lineAddr;
