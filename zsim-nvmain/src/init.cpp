...
static void InitSystem(Config& config) {
	...
	if( config.exists("sys.tlbs"))
	{
		...
		//zinfo->memory_node = new MemoryNode(0,0);	//only one node in system
		...
		//zinfo->buddy_allocator = new BuddyAllocator(zinfo->memory_node);
		...
		//BuddyAllocator::InitMemoryNode(zinfo->memory_node);
		//std::cout<<"number of node_zones:"<<zinfo->memory_node->node_zones.size()<<std::endl;
...
/***----added by YuJieChen on 2015.10.19-----***/
/****-----init memory management-----------****/
//void InitMemoryManagement( Config config )
//{
	/*if( config.exists("sys.tlbs"))
	{
		debug_printf("tlb exists");
		if( config.exists(c_zone_normal) )
			zinfo->max_zone_pfns[Zone_Normal] = (2<<20)*config.get<Address>(c_zone_normal)>>(zinfo->page_shift);
		else
			zinfo->max_zone_pfns[Zone_Normal] = zinfo->memory_size;	//hasn't configured zone , take all memory as zone normal 
		//init max_zone_pfns
		if( config.exists(c_zone_sec))
		{
			//default unit is MB
			if( config.exists(c_zone_dma) )
				zinfo->max_zone_pfns[Zone_DMA] = ((2<<20)*
				config.get<Address>(c_zone_dma))>>(zinfo->page_shift);
	...
	{
		debug_printf("tlb doesn't exist");
		zinfo->memory_node = NULL;
		zinfo->buddy_allocator = NULL;
	}*/
//}
